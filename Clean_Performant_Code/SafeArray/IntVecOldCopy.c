#include "IntVecOldCopy.h"

// ----------------------------------------------------------------------
// helper functions

static int int_vec_expand_(char** data, size_t* length, size_t* capacity, size_t memsz) {
...
static int int_vec_insert_(char** data, size_t* length, size_t* capacity, size_t memsz, size_t idx) {
...
static void int_vec_init(int_vec* vec)
{
...
static void int_vec_free(int_vec* vec)
{
    free(vec->data);
...
*/
static int int_vec_reserve(int_vec* vec, size_t new_capacity)
{
   // int* new_data;
...
*/
static int int_vec_resize(int_vec* vec, size_t new_length)
{
    size_t new_capacity;
...
static int int_vec_push_old(int_vec* ptr, int val)
{
...
//  Push value to end
static int int_vec_push(int_vec* vec, int value)
{
...
//  Remove last element
static int int_vec_pop(int_vec* vec, int* out_value)
{
    if (vec->length == 0) {
        return -1;
    }
...
static int int_vec_insert(int_vec* ptr, size_t idx, int val)
{
...
// Chat GPT
static int int_vec_insertZWEI(int_vec* vec, size_t index, int value)
{
    if (index > vec->length) {
        return -1;
    }
...
static int int_vec_remove(int_vec* vec, size_t index)
{
    if (index >= vec->length) {
        return -1;
    }
...
// Remove all elements
static void int_vec_clear(int_vec * vec)
{
    vec->length = 0;
}
...
** Safe element access
*/
static int* int_vec_get(int_vec* vec, size_t index)
{
    if (index >= vec->length) {
        return NULL;
    }
...
*/
static int int_vec_set(int_vec* vec, size_t index, int value)
{
    if (index >= vec->length) {
        return -1;
    }
...
** }
*/
static int* int_vec_next(int_vec* vec, size_t* index)
{
    int* value;
...
static int int_vec_shrink_to_fit(int_vec* vec)
{
    int* new_data;
...
static void int_vec_truncate(int_vec* ptr, int len)
{
    ptr->length = (len < ptr->length) ? len : ptr->length;
}

static int first(int_vec* ptr)
{
    return ptr->data[0];
}
